...
The generated assets in the local assets directory are converted/reworked visual data for the FreeSpace 2 GTF Hercules. They are created by tools/extract_hercules.py from data in a FreeSpace 2 installation supplied by the user.

This repository intentionally does not contain the original FreeSpace 2 VP, POF, or PCX files, and a release archive must not contain them. The script reads the required entries temporarily and writes only the runtime mesh and texture files required by FS2Hercules.
...
The extractor uses the FreeSpace Open pof-tools crate from a local checkout when Cargo is used. That toolchain remains subject to its own license.
